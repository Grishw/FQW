...
<div class="card p-3 mb-4">
    ...
    {% for plot_index in range(plot_count) %}
        {{plots[plot_index]}}
    {% endfor %}
    <p>Точность прогнозов mse в среднем для интервала в {{day}} = {{day}}*24: {{mse}}</p>
    <p>Точность прогнозов r2 в среднем для интервала в {{day}} = {{day}}*24: {{r2}}</p>
    <p>Точность первого прогноза mse : {{mse_1}}</p>
    <p>Точность первого прогноза r2 : {{r2_1}}</p>
</div>'''
